layui.use(['jquery','layer', 'table','form','upload','element'], function() {
    ...
    var uploadInst = upload.render({
        ...
        ,data: {"path":"/Users/wancong/Pictures/img"} //上传的目标文件夹路径
...
//图片放大镜
    ...
    function updRoomsFlag(roomid,flag){
        $.post(
            "/rooms/updByPrimaryKeySelective", //调用的是base系列的方法，只需要改mapper.xml文件
            {"id":roomid, "flag":flag},
            function (data){
                if(data === 'success'){
                    //重新加载客房数据
    ...
    function checkRoomNum(roomNum){
        ...
        $.post(
            "/rooms/getCountByPramas", //调用的是base系列的方法，只需要改mapper.xml文件
            {"roomNum":roomNum},
            function (data){
                console.log(data);
                if(data == 0){
    ...
    function saveRooms(saveJsonRooms){
        $.post(
            "/rooms/save", //调用的是base系列的方法，只需要改mapper.xml文件
            saveJsonRooms,
            function (data){
                if(data === 'success'){
                    //重新加载客房数据
